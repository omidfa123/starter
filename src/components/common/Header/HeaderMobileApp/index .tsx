...
function HeaderMobileApp() {
  ...
  return (
    <>
    <Box as="header" pt="6" px="9" display="flex" gap="20px" zIndex={isOpen? 0 : 2} pos="relative">
      <SearchMobile isOpen={disclosure.isOpen} onClose={disclosure.onClose} onOpen={disclosure.onOpen} />
      <IconButton
        variant="ghost"
        aria-label="باز کردن منو"
        bgColor="#f5f5f5"
        icon={disclosure.isOpen ? <CloseIcon boxSize={4} color="text"/> :<MenuIcon boxSize={6}  />}
    ...
    <Drawer isOpen={isOpen} onClose={onClose} size="full"   >
        ...
        <Flex justifyContent="space-between"  alignItems="center"  px="9" pb="17.3px" pt="32px" boxShadow=" 0px 4px 12px rgba(0, 0, 0, 0.08)" roundedBottom={32} mb="10">
        ...
        aria-label="باز کردن منو"
        bgColor="#f5f5f5"
        icon={<AngleDownIcon transform={"rotate(90deg)"} color="text" boxSize={5} />}
        onClick={onClose}
        h="43px"
        minW="43px"
        rounded={12}
      /> 
       ...
       <List px="9" spacing="6" mb="12" >
        {list.map(li => (
            ...
            <Text  fontSize="13px" fontWeight="700">{li.text}</Text>
             <ListIcon as={AngleDownIcon} mr="auto" transform={"rotate(90deg)"} />
          </ListItem>
        ))}
        </List>
